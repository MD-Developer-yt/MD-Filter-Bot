...
async def gen_link_s(bot, message):
    ...
    if message.has_protected_content and message.chat.id not in ADMINS:
        return await message.reply("OᴋDᴀ")
...
async def gen_link_batch(bot, message):
    if " " not in message.text:
        return await message.reply("Usᴇ ᴄᴏʀʀᴇᴄᴛ ғᴏʀᴍᴀᴛ.\nExample <code>/batch https://t.me/Anime_UpdatesAU/2 https://t.me/Anime_UpdatesAU/5</code>.")
...
# ------------------------- #
    ...
    if f_chat_id != l_chat_id:
        return await message.reply("Cʜᴀᴛ ɪᴅs Nᴏᴛ Mᴀᴛᴄʜᴇᴅ 🔎.")
    try:
...
# ------------------------- #
    ...
    post = await bot.send_document(LOG_CHANNEL, f"batchmode_{message.from_user.id}.json", file_name="Batch.json", caption="⚠️ Gᴇɴᴇʀᴀᴛᴇᴅ Fᴏʀ FɪʟᴇSᴛᴏʀᴇ.")
